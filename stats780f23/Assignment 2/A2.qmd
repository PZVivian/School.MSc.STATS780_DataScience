...
The goal of this study is to predict customer churn at a bank. The data was downloaded from Kaggle [@bankData], which originally consisted of `r ncol(bankRaw)` variables and `r nrow(bankRaw)` rows of data related to customer churn and details about the customer such as age, bank balance, gender, and more. A binary variable indicating whether a customer churned or not was used as the response variable. Row numbers, customer ids, and surnames were removed from the dataset because they are not important for the purpose of classification. Based on Harrell's 1:15 rule of choosing predictor variables with respect to sample size [-@Harrell], the remaining 10 variables were used as predictor variables for classification. A full description of each variable along with their data types can be found in the Supplementary Materials section.
