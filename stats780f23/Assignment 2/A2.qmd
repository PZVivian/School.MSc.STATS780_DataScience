...
corr_matrix <- cor(bank %>% dplyr::select(-Geography, -Gender, -HasCrCard, -IsActiveMember, -Exited))
...
log_mod1_mcerate <- mean(log_mod1_y != test_y) # Miss-classification error rate (% of churn incorrectly predicted)
log_mod1_accuracy <- mean(log_mod1_y == test_y) # Accuracy (% of churn correctly predicted)
log_mod1_sensitivity <- log_mod1_cmatrix[2,2]/sum(log_mod1_cmatrix[2,]) # Sensitivity (% correctly predicted as churned)
log_mod1_specificity <- log_mod1_cmatrix[1,1]/sum(log_mod1_cmatrix[1,]) # Specificity (% correctly predicted as not churned)
log_mod1_stats <- c(log_mod1_mcerate, log_mod1_accuracy, log_mod1_sensitivity, log_mod1_specificity)

# --- KNN MODEL PERFORMANCE --- #
# Stats on model performance
knn_mod1_cmatrix <- table(knn_mod1_y, test_y) # Confusion matrix
knn_mod1_mcerate <- mean(knn_mod1_y != test_y) # Miss-classification error rate (% of churn incorrectly predicted)
knn_mod1_accuracy <- mean(knn_mod1_y == test_y) # Accuracy (% of churn correctly predicted)
knn_mod1_sensitivity <- knn_mod1_cmatrix[2,2]/sum(knn_mod1_cmatrix[2,]) # Sensitivity (% correctly predicted as churned)
knn_mod1_specificity <- knn_mod1_cmatrix[1,1]/sum(knn_mod1_cmatrix[1,]) # Specificity (% correctly predicted as not churned)
knn_mod1_stats <- c(knn_mod1_mcerate, knn_mod1_accuracy, knn_mod1_sensitivity, knn_mod1_specificity)

# --- COMPARE MODELS --- #
comparison <- data.frame(row.names = c("Miss-classification error rate", "Accuracy", "Sensitivity", "Specificity"),
