...
In Islam et al.'s study [-@diabetesStudy], machine learning techniques were used to predict diabetes risk. In particular, naive Bayes, logistic regression, and random forest techniques were applied. The study found that their random forest model had the best accuracy [@diabetesStudy]. 

The study we propose will also utilize machine learning techniques, but will focus on decision tree and neural network methods instead.


## Methods
As a starting point, the data will be randomly split into two equal portions as the training and testing set.

The first machine learning method will be a decision tree. The index on the subtrees $\alpha$ will be tuned using cross-validation. Pruning will be applied to reduce the cost complexity of the tree, and random forest ensembling will be used to improve model performance.

The second method will be a neural network. The number of hidden layers and the units per layer will be tuned through trial and error. Based on James et al.'s book on statistical learning [@introToStatsLearning], the units per layer will be set to some large value and overfitting will be controlled with ridge regularization. The strength of the regularization $\lambda$ will be tuned at each layer.

After the two models are built, they will each be assessed using misclassification rate, accuracy, specificity, and sensitivity. A comparison of these four measurements will reveal which model is a stronger fit to predict diabetes.
...
@fig-barcharts shows a bar chart for each of the 16 categorical variables in the data set. There appears to be a somewhat even split between the predictor values for polyuria and itching, while the remaining predictors are not evenly split. This is especially important for the class variable because there are 200 observations with a negative diabetes risk and 300 observations with a positive risk. This suggests that if the model does not perform well, a resampling method such as cross-validation or boostrapping may help improve the model.
...
```{r fig-boxplot, ref.label="boxplot", output=TRUE, message=FALSE, fig.cap="Boxplot of age", fig.height=3, fig.width=3}
```
