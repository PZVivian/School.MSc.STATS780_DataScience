...
tree_summary <- data.frame("Model" = model, 
                           ...
                           "Misclassification Rate" = as.3percent(misclass),
                           check.names = FALSE)
kable(tree_summary)
```
...
```{r fig-tree-rforest-imp, ref.label="tree-rforest-imp", output=TRUE, message=FALSE, fig.cap="Importance of variables from random forest", fig.height=5, fig.width=8}
```
