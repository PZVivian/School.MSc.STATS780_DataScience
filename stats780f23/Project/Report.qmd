...
```{r fig-tree-fit1-cv, ref.label="tree-fit1-cv", output=TRUE, message=FALSE, fig.cap="Cross validation of first decision tree"}
```
...
```{r fig-svm-linear-cv, ref.label="svm-linear-cv", output=TRUE, message=FALSE, fig.cap="Linear kernel cross validation"}
```
```{r fig-svm-poly-cv, ref.label="svm-poly-cv", output=TRUE, message=FALSE, fig.cap="Polynomial kernel cross validation"}
```
```{r fig-svm-radial-cv, ref.label="svm-radial-cv", output=TRUE, message=FALSE, fig.cap="RBF kernel cross validation"}
```
:::
