...
# Pull out classes for testing (vector version)
diabetesTest_class <- diabetes$class[-trainIndex]

# Pull out classes for testing (matrix version)
classLabs <- pull(diabetes, class) %>% class.ind()
diabetesTrain_class_mat <- classLabs[trainIndex,]
diabetesTest_class_mat <- classLabs[-trainIndex,]
...
```{r network, output=FALSE}
# Use trial and error to find the size and decay that produces a minimal test error rate.

## TRY 1 ##
# Build a single layer neural network
diabetesNN <- nnet(
  x = diabetesTrain,
  y = diabetesTrain_class_mat,
  size = 1, 
  decay = 2, 
  softmax = TRUE)
# Predict using the neural network and compute the test error rate
diabetesNN_pred <- predict(diabetesNN, diabetesTest, type="class")
tab <- table(slice(diabetes, -trainIndex) %>% pull(class), diabetesNN_pred)
...
modelnn <- nn_module(
  initialize = function() {
    self$linear1 <- nn_linear(in_features = 17, out_features = 8)
    self$linear2 <- nn_linear(in_features = 8, out_features = 4)
    self$linear3 <- nn_linear(in_features = 4, out_features = 2)
  ...
  forward = function(x) {# specify the order to call the layers
    x %>% 
...
```{r}
# Change to torch data set
diabetesTrain_tensor <- torch_tensor(data.matrix(diabetesTrain)-1, dtype = torch_float())

# Run neural network
system.time(
   fitted <- modelnn %>%
      fit(
        data = diabetesTrain_tensor, 
        epochs = 2, # max epochs
        valid_data = 0.5,
        dataloader_options = list(batch_size = 20),#how many samples per batch to load
        verbose = FALSE
      )
 )
plot(fitted)
...
truth <- sapply(
  seq_along(test_ds), function(x) test_ds[x][[2]]
  )
...
```{r}
write.csv(diabetesTrain, "diabetesTrain.csv", row.names=FALSE)
write.csv(diabetesTest, "diabetesTest.csv", row.names=FALSE)
```
...
```{r}
plot(svmfit, diabetesTrain_Scaled)
```
