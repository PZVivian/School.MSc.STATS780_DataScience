...
Finally, the SVM model with an RBF kernel had an accuracy of `r as.percent(radial_accuracy)`, which is the highest value among the three kernels. In the clinical setting, this means about 2 out of 100 patients would be incorrectly classified in terms of diabetes risk, which is a decent improvement compared to the linear kernel model. Its specificity is slightly lower than the polynomial kernel model, but its sensitivity is higher by about `r as.percent(radial_sensitivity - poly_sensitivity)`.
...
- All of the models had very high accuracy, but in the context of the data minimizing misclassification is crucial to deem the prediction plausible for clinical use
- 
