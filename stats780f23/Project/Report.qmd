...
After building each model, we computed the accuracy, specificity, sensitivity, and misclassification error rate to assess model performance. These performance parameters were selected because they are often used for binary classification problems. Moreover, a few of these metrics were used by Islam et al. [-@diabetesStudy], so computing them allows us to directly compare our results with theirs. Comparing these four measurements between each of the methods helps us determine which model is a stronger fit to predict diabetes.
...
The random forest had the best performance among the three models with an accuracy of `r as.percent(dForest_accuracy)`. This means about 1 out of 100 patients would be incorrectly classified, which is a large improvement compared to the initial tree. Its ability to predict cases of diabetes risk and cases without diabetes risk is nearly even. Additionally, we were also able to obtain a more rigorous analysis on variable importance through the random forest. We found that the most important variables in predicting diabetes include polyuria, polydipsia, gender, and age as shown in @fig-tree-rforest-imp. This is determined using both the mean decrease in accuracy and Gini index. Checking for these four variables may help healthcare professionals identify patients with a greater risk for the disease and facilitate early diagnosis.
...
Overall, each of the models in our study achieved an accuracy in the 90% to 99% range. While this may seem high, minimizing misclassification error is paramount from a medical perspective since patient health is highly dependent on accurate disease assessment. For this reason, many of the models we developed may not provide enough accuracy to be used as the sole predictive tool for diabetes in clinical practice. However, combining the predictions and important variables from these models along with other tools may be useful.

@tbl-final-summary summarizes the top two models in our study. From the table, we can see that the random forest outperforms SVM for all parameters except specificity. As a result, we can conclude that our random forest model is our highest performing model. This aligns with Islam et. al's study [-@diabetesStudy] since they also found that their random forest methods had the highest performance. They were able to achieve an accuracy of 97% using ten-fold cross-validation and 99% using 80:20 percentage split for their random forest models [@diabetesStudy]. These results are very similar to our random forest accuracy of `r as.percent(dForest_accuracy)`. Thus, we can say that our model results are consistent with current literature and that decision trees, specifically random forests, remain as the most accurate method for diabetes risk prediction.
